...
let province = ChineseDistricts['86'];
let spilt = '\r';

for (let key in province) {
    let provinceAry = province[key];
    // console.log(provinceAry)

    for (let {code, address} of provinceAry) {
        console.log(code, address);
        write(address + spilt);

        let city = ChineseDistricts[code];
        // console.log(city);
        for (let tmp in city) {

            let cityCode = tmp;
            let cityAddress = city[tmp];
            console.log(cityCode, cityAddress);
            write(cityAddress + spilt);


            let district = ChineseDistricts[cityCode];
            // console.log(district);
            for (let tmp in district) {
                let districtCode = tmp;
                let districtAddress = district[tmp];
                console.log(districtCode, districtAddress);
                write(districtAddress + spilt);
            }
        }
    }
}
